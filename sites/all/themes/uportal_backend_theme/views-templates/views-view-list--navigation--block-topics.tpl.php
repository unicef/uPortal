...
$path_to_theme = drupal_get_path('theme', 'ugyouthportal_theme');

$home_icon = theme_image(array(
	'path' => $path_to_theme.'/images/icon-menu-home.png',
	'attributes' => array(),
));
$index_menu_icon = theme_image(array(
	'path' => $path_to_theme.'/images/icon-menu-title-index-white.png',
	'attributes' => array(),
));
...
//get topics with published content
$topics_with_content_view = views_get_view_result('navigation', 'block_active_topics');
$topics_with_content = array();
foreach ($topics_with_content_view as $topic_view_result) {
	$topic_nid = $topic_view_result->node_field_data_field_topic_reference_nid;
	$topic_title = $topic_view_result->node_field_data_field_topic_reference_title;
	$topics_with_content['nid-'.$topic_nid] = $topic_title;
}
...
for ($i=0; $i<$results_no; $i++) {
	...
	if (!isset($topics_with_content['nid-'.$topic_nid])) {
		continue;
	}
...
$index_link = l(
	'<span class="img">'.$index_menu_icon.'</span><span class="title">Content Index</span>',
	$all_url['url'],
	$all_url['options']
);
